# Git AutoCRLF 設定変更・復旧ツール

このツールは、既存の複数Gitリポジトリで`core.autocrlf=false`への設定変更を**安全かつ簡単**に行うためのツールです。

## 特徴

- **安心**: コミット作業不要、全自動で安全に処理
- **シンプル**: バッチファイルは1つだけ
...
4. **設定変更** - `core.autocrlf=false` を設定
5. **HEADリセット** - GitキャッシュをクリアしてワーキングディレクトリをHEADの状態に復元
6. **整理** - 一時的に生成されたファイルのみを整理（重要なファイルは保護されます）

**コミット作業は一切不要**です。すべて自動で安全に処理されます。

## ファイル構成

```
git-maintenance/
├── git-autocrlf-recovery.bat  # メインツール
...
`git-autocrlf-recovery.bat` をダブルクリックして実行します。

実行時に以下のメニューが表示されます：

```
1. AutoCRLF設定変更・復旧実行    ← メイン機能
2. リポジトリ情報確認            ← 実行前の確認用
3. スタッシュ一覧表示            ← スタッシュ確認用
4. git-autocrlf-recovery関連スタッシュ復元  ← 元に戻したい時
5. 終了
```
...
- **HEADリセット**: 改行コード変更による差分は自動でHEADの状態に戻されます
...
```
log/git-autocrlf-recovery_YYYYMMDD_HHMMSS.log
```
